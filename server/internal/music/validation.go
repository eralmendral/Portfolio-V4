...
func validateCreate(input CreateMusicRequest) error {
	...
	if strings.TrimSpace(input.Artist) == "" {
		fields["artist"] = "artist is required"
	}
	validateDate("mostly_listened_on", input.MostlyListenedOn, true, fields)
	validateURL("spotify_url", input.SpotifyURL, fields)
	validateURL("youtube_url", input.YouTubeURL, fields)
...
func validateUpdate(input UpdateMusicRequest) error {
	fields := make(map[string]string)

	if input.Title != nil && strings.TrimSpace(*input.Title) == "" {
		fields["title"] = "title cannot be blank"
	}
	if input.Artist != nil && strings.TrimSpace(*input.Artist) == "" {
		fields["artist"] = "artist cannot be blank"
	}
