...
func validateCreate(input CreateSeriesRequest) error {
	fields := make(map[string]string)

	if strings.TrimSpace(input.Title) == "" {
		fields["title"] = "title is required"
	}
	validateCategory("category", input.Category, true, fields)
	validateDate("mostly_watched_on", input.MostlyWatchedOn, true, fields)
	validateURL("watch_url", input.WatchURL, fields)
...
func validateUpdate(input UpdateSeriesRequest) error {
	fields := make(map[string]string)
...
func validateListFilter(filter ListFilter) error {
	...
	validateCategory("category", filter.Category, false, fields)
...
func validateCategory(field string, value string, required bool, fields map[string]string) {
	if strings.TrimSpace(value) == "" {
		if required {
			fields[field] = "category is required"
		}
		return
	}

	switch normalizeCategory(value) {
	case CategoryTVSeries, CategoryAnime:
	default:
		fields[field] = "category must be tv_series or anime"
	}
}
